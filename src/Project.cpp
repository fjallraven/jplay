...
// v8: new data model — Media has id/name/version/department/assetVariation;
//     Clips are owned by Sequences; Shots have no color or clipIds.
//     Drops all backward compat with v1-v7.
// v9: Shots gain originalCutIn/originalCutOut/cutDiffer (track edited cut ranges).
// v10: clip/shot timelineStart stored relative to the owning sequence's region
//      (per-sequence-local frames); repackSequences() restores absolute layout
//      on load. v9 files (always single-sequence) still load via the same path.
// v11: Media metadata is a key/value map keyed by naming-config picker keys,
//      replacing the fixed version/department/assetVariation fields. v9/v10 files
//      still load: their three fields map to keys version/department/asset.
// v12: Media caches its applicable-picker set (path-derived; resolved once via
//      Python) so updateMetaBar need not re-parse the path each frame. Older
//      files load with it unresolved and resolve it lazily on first display.
// v13: Clips gain a `hidden` flag (disabled clips are skipped in compositing so a
//      lower track shows through). Older files load with hidden = false.
// v14: Trailing i32 viewSeqIdx records the sequence view filter that was active
//      (-1 = All). Older files load with it defaulted to -1 (playhead/current
//      frame was already persisted since v8).
// v15: Media gains an external audio track (audioPath + source-frame audioOffset)
//      so silent EXR sequences can carry sound. Older files load with none.
// v16: Clips carry pencil annotations — a map of source frame -> stroke list,
//      appended after the clip's hidden flag. Each stroke is a captured rgb color
//      plus a list of image-space points (x, y, half-width). Older files load with
//      no annotations.
// v17: Trailing f64 letterboxRatio + f32 letterboxOpacity record the letterbox
//      matte (aspect ratio W/H, 0 = off; and bar opacity). Written after
//      viewSeqIdx. Older files load with letterbox off.
// v18: Audio tracks. i32 audioTrackCount follows trackCount; clips gain a u8
//      audio flag (after hidden) placing them in the audio track index space.
//      Media loses the v15 external-audio fields (audioPath/audioOffset) —
//      attached audio is replaced by audio clips. Drops all backward compat:
//      only v18 files load.
// v19: Unified track model. Video and audio share a single track index space
//      (one row = one type, decided by its clips), so audioTrackCount is gone —
//      only trackCount follows the timeline bounds. The per-clip u8 audio flag
//      stays (it names the track's type). Drops all backward compat: only v19
//      files load.
// v20: Trailing u8 ocioEnabled records the project's color-management mode
//      (1 = OCIO display transform, 0 = built-in linear→sRGB). Written after
//      letterboxOpacity. Older files load with it defaulted to on (OCIO).
// v21: Shots gain a sceneName string (from OTIO "scene_name"), written after
//      the shot's name. Older files load with it empty.
// v29: A project is a record, not a name. The timeline gains a project table
//      (id / name / path / openedWhole) written in the trailer, sequences store an
//      i32 projectId into it in place of the v22 name string, and the view scope
//      stores that id in place of the v27 name. Older files migrate: one record per
//      distinct name, with openedWhole taken from the v27 opened-projects list.
//      Identity moves to the file path, so two projects whose stems match no longer
//      collapse into one.
// v22: Sequences gain a projectName string (the stem of the .otio they were
//      imported from), written after the sequence's name. Older files load with it
//      empty, which leaves those sequences ungrouped in the Sequence popup.
// v23: Sequences gain a u32 bgColor (packed 0xRRGGBB timeline sequence-bar fill),
//      written after the sequence's projectName. Older files load with the random
//      color every fresh Sequence starts with, which sticks once resaved.
// v24: Sequences gain a transition list (dissolves on their cuts), written after
//      the sequence's clips. Older files load with none, which is the fresh state.
// v25: Clips gain fadeInFrames / fadeOutFrames (head/tail opacity ramps), written
//      after the clip's audio flag. Older files load with no fades.
// v26: Clips gain linkedTo / linkOffset (an audio clip follows a video clip),
//      written after the fades. Older files load unlinked: the pairing in them was
//      implicit (audio on the row below, same span) and is deliberately not
//      inferred, so those projects behave exactly as they did until relinked.
// v27: The view scope gains its project half — a trailing u32+str viewProjName (the
//      project the view was scoped to, empty = none) plus the list of projects that
//      were opened whole, written after ocioEnabled. Older files load with neither,
//      so they keep landing on the sequence named by viewSeqIdx.
// v28: Media gains a f32 pixelAspect (pixel width / height), written after its
//      fps. Older files load with 1 (square), which every source but an
//      anamorphic EXR is; a re-probe of the media fills in the real value.
// v30: Custom track labels - a u32 count plus that many strings, one per track
//      row, written after the project table. An empty string is a row with no
//      custom name; older files load with none, so every row keeps its derived
//      "Video 1" / "Audio 1" / "Track N" label.
// v31: Disabled track rows - a u32 count plus that many u8 flags, one per track
//      row, written after the track labels. Non-zero switches the row off (its
//      clips are skipped in compositing and audio, as the eye in the track header
//      does); older files load with every row enabled.
// v32: Media gains a u32+str colour space — the user's explicit choice of the OCIO
//      input space its pixels are read in — written after its picker keys. Empty
//      means the space is resolved from the config (its file rules, then the
//      container's own colour tags); older files load empty, so they open with
//      every media colour-managed by whatever the active config resolves.
// v33: Trailing u32+str ocioView records the OCIO view transform the project was
//      last reviewed through, written after the disabled-track flags. Empty (as
//      every older file loads) means the config's default view, which is what the
//      session picked before it was stored. Only the view is kept: the display
//      names the monitor in front of you rather than anything about the project,
//      and a look is a per-shot decision the config already carries.
// v34: Clips gain a volume automation curve, written after their annotations: a
//      f32 base (dB, 0 = unity), a u8 interpolation mode, and a u32-counted list
//      of (i64 source frame, f32 dB) breakpoints. Older files load a default
//      Curve, which is exactly unity — the gain every clip had before.
// v35: Trailing str proxyMode records the global Proxy dropdown's last
//      selection ("" = "Full"), written after ocioView. Older files load with
//      it empty, i.e. Full — no substitution.
