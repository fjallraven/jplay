...
bool App::init(int argc, char** argv) {
    ...
    // OpenEXR's own pool, which a lone EXR read (the first frame, a scrub target)
    // spreads its decompression across; reads that overlap don't use it (see
    // ExrSource.cpp). Before the first read so that frame gets it. A 2K ZIP frame
    // measured 21 ms without, 8 ms with 4 threads, 6 ms with 16 (tests/exrbench).
    Imf::setGlobalThreadCount(std::clamp((int)std::thread::hardware_concurrency(), 1, 16));
